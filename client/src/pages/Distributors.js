...
const Distributors = () => {
  ...
  if (error) {
    ...
    return <div className="text-red-500 text-center p-4">Error loading distributors</div>;
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold text-gray-900">Distributors Management</h1>
          <p className="mt-2 text-sm text-gray-600">
            Manage distributor accounts and information
          </p>
        </div>
        <button
          ...
          <div className="p-6">
            <div className="flex items-center">
              ...
              <div className="ml-4 w-0 flex-1">
                <dl>
                  <dt className="text-sm font-medium text-gray-500 truncate">Total Distributors</dt>
        ...
        <div className="p-6">
          <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between space-y-4 lg:space-y-0">
            <div className="flex-1 max-w-lg">
              <label htmlFor="search" className="sr-only">Search distributors</label>
        ...
        {filteredDistributors.length > 0 ? (
            ...
            <table className="min-w-full divide-y divide-gray-200">
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {filteredDistributors.map((distributor) => (
                  <tr key={distributor.id} className="hover:bg-gray-50 transition-colors duration-150">
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="flex items-center">
                        ...
                        <div className="ml-4">
                          ...
                          <div className="text-sm text-gray-500">ID: {distributor.id}</div>
                        </div>
                      </div>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="text-sm font-medium text-gray-900">{distributor.company_name}</div>
                      <div className="flex items-center text-sm text-gray-500 mt-1">
                        <MapPinIcon className="h-4 w-4 mr-1" />
                        {distributor.address}
                      </div>
                      <div className="text-sm text-gray-500">GST: {distributor.gst_number}</div>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="flex items-center">
                        {getStatusIcon(distributor.status)}
                        <span className="ml-2">{getStatusBadge(distributor.status)}</span>
                      </div>
                    </td>
          ...
          <div className="text-center py-12">
            ...
            <p className="mt-1 text-sm text-gray-500">
              {searchTerm ? 'Try adjusting your search terms.' : 'Get started by adding your first distributor.'}
            </p>
          </div>
        )}
      </div>
